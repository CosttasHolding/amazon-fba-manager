...
| Schema prod vs código | columnas reales tablas webhook | query read-only service-role | código asume org_id | **org_id NO existe** en `sp_api_webhook_subscriptions` (42703) ni `sp_api_webhook_logs` (PGRST204) → select de suscripción SIEMPRE falla → ninguna notificación se atribuiría jamás, incluso con seller conectado | FAIL | 2026-08-22 | repro node local contra prod |
| Integraciones | sp_api_connections | select limit 1 | descubrir estado | **tabla vacía** → SP-API NOT CONFIGURED (webhooks sin tráfico real hoy; mismatch es latente) | NOT CONFIGURED | 2026-08-22 | ídem |

### Fix propuesto para schema mismatch (requiere aprobación owner — cambio de schema en prod)

Migración 037: `ALTER TABLE sp_api_webhook_subscriptions ADD COLUMN IF NOT EXISTS org_id uuid REFERENCES organizations(id);` e igual para `sp_api_webhook_logs`. Ambas tablas están vacías en prod → cero riesgo de datos. Alternativa código-side (derivar org vía connection_id→sp_api_connections.org_id) descartada porque complica cada query y la columna es lo que todo el código ya espera.
