...
export const HELP_GLOSSARY: { term: string; definition: string }[] = [
  ...
  {
    term: "Gobierno Corporativo (Governance)",
    definition:
      "Sistema de gestión de socios de la LLC: miembros, participaciones, albaceas, tareas, eventos de sucesión y decisiones directivas. Diseñado para cumplir con el Operating Agreement de Costtas Holding.",
  },
  {
    term: "Miembro (Member)",
    definition:
      "Socio de la LLC con participación accionaria. Cada miembro tiene nombre, email, porcentaje de ownership, estado (activo/fallecido/retirado) y albacea asignado para planificación sucesoria.",
  },
  {
    term: "Participación (Ownership)",
    definition:
      "Porcentaje del capital social de la LLC que posee un miembro. La suma de todas las participaciones debe ser 100%. Se usa para calcular distribución de ganancias y precio de transferencia en sucesión.",
  },
  {
    term: "Albacea (Executor)",
    definition:
      "Persona designada por un miembro para gestionar su patrimonio en caso de fallecimiento. Crítico para el plan de sucesión. Debe tener nombre y email registrados en el sistema.",
  },
  {
    term: "Tablero Kanban (Tasks)",
    definition:
      "Sistema de gestión de tareas con columnas Pendiente → En Progreso → Completada. Soporta drag & drop, prioridades (Baja/Alta/Urgente), asignación a miembros, fechas de vencimiento y módulos (Documentos, FBA, General).",
  },
  {
    term: "Valuación (Art. 11.2)",
    definition:
      "Cálculo del valor de empresa para determinar precio de transferencia de participación. Fórmula: (Ingreso Neto Promedio 36 meses × Múltiplo 3x-5x) + Activos Netos Ajustados. Precio de Transferencia = Valor Empresa × (% Participación / 100).",
  },
  {
    term: "DEC (Board Decision)",
    definition:
      "Decisión del Directorio. Documento formal que registra resoluciones directivas con estado (borrador/aprobado/rechazado/ejecutado), referencia a documento, votos y fecha de decisión.",
  },
  {
  ...
  },
  {
    ...
    definition:
      "Sistema de alertas in-app con prioridades (baja/media/alta/crítica). Notifica sobre stock bajo, tareas vencidas, eventos de sucesión y cambios en decisiones directivas. Indicador de notificaciones no leídas en el header.",
  },
  {
    term: "Búsqueda Global (Cmd+K)",
    definition:
      "Paleta de comandos 'Command K' para búsqueda en toda la app. Accesible con Cmd+K (Mac) o Ctrl+K (Windows). Busca productos, proveedores, pedidos y módulos, con acceso directo a resultados y teclas de atajo.",
  },
  {
    term: "Onboarding",
    definition:
...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  // ===== AMAZON API (SP-API) =====
  {
    ...
    kpis: [
      { label: "Conexiones", description: "Marketplaces conectados via OAuth SP-API" },
      { label: "Última Sincronización", description: "Timestamp del último sync completo" },
      { label: "Sync Activo", description: "Indica si hay un sync en ejecución" },
    ],
    tables: [
      {
        label: "Historial de Sincronización",
        columns: [
          { name: "Tipo", description: "products / orders / inventory / fees / returns / payouts" },
          { name: "Estado", description: "success / error / running" },
          { name: "Procesados", description: "Items procesados en el sync" },
          { name: "Fallidos", description: "Items con error" },
          { name: "Fecha", description: "Timestamp de ejecución" },
        ],
      },
    ],
    actions: [
      { label: "Conectar Marketplace", description: "OAuth flow para conectar Seller Central" },
      { label: "Sincronizar Todo", description: "Ejecuta sync completo de todos los tipos" },
      { label: "Sincronizar por Tipo", description: "Sync selectivo: solo productos, órdenes, etc." },
    ],
    glossary: [
      { term: "OAuth SP-API", definition: "Autenticación delegada via Amazon. El usuario autoriza a la app a acceder a su cuenta de Seller Central. El refresh token se almacena para renovación automática." },
      { term: "Sync Products", definition: "Obtiene listings activos via getListings() y actualiza/crea productos en la DB." },
      { term: "Sync Orders", definition: "Obtiene órdenes recientes via getOrders() + getOrderItems() y las registra como ventas." },
      { term: "Sync Inventory", definition: "Obtiene niveles de stock FBA via getInventory() y actualiza inventory + alertas." },
      { term: "Sync Fees", definition: "Estima tarifas FBA y referral fee para cada producto via getFeeEstimate()." },
      { term: "Sync Returns", definition: "Obtiene reportes de devoluciones via Reports API y registra returns." },
      { term: "Sync Payouts", definition: "Obtiene reportes de pagos via Reports API para el módulo de finanzas." },
    ],
    tips: [
  ...
  // ===== MIEMBROS (GOVERNANCE) =====
  {
    ...
    kpis: [
      { label: "Total Miembros", description: "Cantidad total de socios registrados" },
      { label: "Activos", description: "Miembros con estado Activo" },
      { label: "Participación Total", description: "Suma de ownership de todos los miembros (debe ser ~100%)" },
      { label: "Albaceas Asignados", description: "Miembros que tienen albacea designado" },
    ],
    tables: [
      {
        label: "Lista de Miembros",
        columns: [
          { name: "Nombre", description: "Nombre completo del socio" },
          { name: "Email", description: "Correo electrónico del miembro" },
          { name: "Participación", description: "Porcentaje de ownership en la LLC" },
          { name: "Estado", description: "Activo / Retirado / Fallecido" },
          { name: "Albacea", description: "Nombre del albacea designado" },
        ],
      },
    ],
    actions: [
      { label: "Nuevo Miembro", description: "Formulario para registrar nuevo socio" },
      { label: "Ver Detalle", description: "Información completa del miembro" },
      { label: "Editar", description: "Modificar datos del miembro" },
      { label: "Eliminar", description: "Elimina miembro con confirmación" },
    ],
    glossary: [
    ...
    forms: [
      {
        ...
        fields: [
          { name: "Nombre Completo", description: "Nombre y apellido del socio", required: true },
          { name: "Email", description: "Correo electrónico del miembro", required: false },
          { name: "Participación (%)", description: "Porcentaje de ownership, valor entre 0 y 100", required: true },
          { name: "Estado", description: "Activo / Retirado / Fallecido", required: true },
        ],
      },
      {
        label: "Datos de Sucesión (Albacea)",
        fields: [
          { name: "Nombre del Albacea", description: "Persona designada para gestionar herencia", required: false },
          { name: "Email del Albacea", description: "Correo del albacea para notificaciones", required: false },
        ],
      },
      {
        label: "Notas",
        fields: [
          { name: "Notas", description: "Información adicional relevante del miembro", required: false },
        ],
      },
    ],
    tips: [
  ...
  // ===== DETALLE MIEMBRO =====
  {
    ...
    kpis: [
      { label: "Participación", description: "Porcentaje de ownership del socio" },
      { label: "Estado", description: "Situación actual del miembro" },
      { label: "Albacea", description: "Nombre del albacea designado" },
    ],
    actions: [
      { label: "Editar", description: "Modificar datos del miembro" },
      { label: "Eliminar", description: "Eliminar miembro con confirmación" },
    ],
    tips: [
    ...
    tables: [
      {
        ...
        columns: [
          { name: "Título", description: "Nombre de la tarea" },
          { name: "Prioridad", description: "Baja / Media / Alta / Urgente (indicador de color)" },
          { name: "Módulo", description: "Documentos / FBA / General" },
          { name: "Asignado a", description: "Miembro responsable" },
          { name: "Fecha Límite", description: "Fecha de vencimiento" },
        ],
      },
      {
        ...
        columns: [
          { name: "Título", description: "Nombre de la tarea" },
          { name: "Prioridad", description: "Baja / Media / Alta / Urgente" },
          { name: "Asignado a", description: "Miembro responsable" },
          { name: "Vencimiento", description: "Días restantes o vencida" },
        ],
      },
      {
        ...
        columns: [
          { name: "Título", description: "Nombre de la tarea" },
          { name: "Asignado a", description: "Miembro que la completó" },
          { name: "Módulo", description: "Categoría de la tarea" },
        ],
      },
    ],
    actions: [
      { label: "Nueva Tarea", description: "Toggle de formulario inline para crear tarea" },
      { label: "Editar", description: "Pre-llena formulario con datos existentes" },
      { label: "Eliminar", description: "Elimina tarea con confirmación" },
      { label: "Drag & Drop", description: "Arrastrar tarjeta entre columnas para cambiar estado" },
    ],
    forms: [
      {
        label: "Crear / Editar Tarea",
        fields: [
          { name: "Título", description: "Nombre descriptivo de la tarea", required: true },
          { name: "Estado", description: "Pendiente / En Progreso / Completada", required: true },
          { name: "Prioridad", description: "Baja / Media / Alta / Urgente", required: true },
          { name: "Módulo", description: "Documentos / FBA / General (o texto libre)", required: false },
          { name: "Fecha Límite", description: "Fecha de vencimiento opcional", required: false },
          { name: "Asignado a", description: "Miembro responsable (desplegable con lista de miembros)", required: false },
          { name: "Descripción", description: "Detalle ampliado de la tarea", required: false },
        ],
      },
    ],
    glossary: [
  ...
  {
    id: "board-decisions",
    title: "Decisiones Directivas",
    route: "/board-decisions",
    description:
    ...
    tables: [
      {
        label: "Decisiones",
        columns: [
          { name: "Título", description: "Nombre de la decisión directiva" },
          { name: "Referencia Documental", description: "Documento asociado en Drive" },
          { name: "Estado", description: "Borrador / Aprobado / Rechazado / Ejecutado" },
          { name: "Fecha Decisión", description: "Fecha de la resolución" },
        ],
      },
    ],
    actions: [
    ...
    glossary: [
      ...
      { term: "Estados", definition: "Borrador = en preparación. Aprobado = vigente. Rechazado = no aprobado. Ejecutado = implementado." },
    ],
    tips: [
    ...
    tables: [
      {
        ...
        columns: [
          { name: "Nombre", description: "Nombre del archivo o carpeta" },
    ...
    tables: [
      {
        ...
        columns: [
          { name: "Prioridad", description: "Crítica / Alta / Media / Baja (indicador de color)" },
          { name: "Mensaje", description: "Contenido de la notificación" },
          { name: "Módulo", description: "Area de la app que genera la alerta" },
          { name: "Fecha", description: "Timestamp de generación" },
          { name: "Leída", description: "Indica si fue vista por el usuario" },
        ],
      },
    ],
    glossary: [
      { term: "Prioridades", definition: "Crítica (rojo) = acción inmediata requerida. Alta (ámbar) = atención pronto. Media (azul) = informativo. Baja (gris) = recordatorio." },
    ],
    tips: [
    ...
    tables: [
      {
        ...
        columns: [
          { name: "Tipo", description: "Producto / Proveedor / Pedido / Módulo" },
          { name: "Resultado", description: "Nombre o título del elemento encontrado" },
          { name: "Acción", description: "Link directo al elemento seleccionado" },
        ],
      },
    ],
    glossary: [
      { term: "Teclas Rápidas", definition: "Cmd+K (Mac) o Ctrl+K (Windows) para abrir. Flechas para navegar. Enter para seleccionar. Esc para cerrar." },
    ],
    tips: [
  ...
  // ===== ONBOARDING =====
  {
    ...
    actions: [
      { label: "Crear Perfil", description: "Configurar nombre, empresa y preferencias en Settings" },
      { label: "Configurar Defaults", description: "Establecer valores por defecto FBA: fees, marketplace, moneda" },
      { label: "Agregar Primer Producto", description: "Registrar un producto en el catálogo" },
      { label: "Agregar Proveedor", description: "Registrar un proveedor en la base" },
      { label: "Importar Ventas", description: "Subir CSV con ventas históricas" },
    ],
    glossary: [
      { term: "Progreso", definition: "El checklist muestra el porcentaje completado. Desaparece automáticamente al 100%." },
    ],
    tips: [
...
export const ROUTE_TO_SECTION: Record<string, string> = {
  ...
  "/import": "import",
  "/members": "members",
  "/members/new": "member-form",
  "/members/[id]": "member-detail",
  "/members/[id]/edit": "member-form",
  "/tasks": "tasks",
