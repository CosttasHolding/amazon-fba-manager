...
function formatCellValue(value: unknown, format?: string, locale: string = "es-ES"): string {
  ...
  switch (format) {
    ...
    case "date": {
      if (value instanceof Date) {
        return value.toLocaleDateString(locale === "en" ? "en-US" : "es-ES");
      }
      const d = new Date(String(value));
      return isNaN(d.getTime()) ? String(value) : d.toLocaleDateString(locale === "en" ? "en-US" : "es-ES");
    }
    case "number":
      return typeof value === "number" ? value.toLocaleString(locale === "en" ? "en-US" : "es-ES") : String(value);
    default:
      return String(value);
  }
}
...
export function exportToExcelPro(
  ...
  const rows = data.map((row) =>
    filteredColumns.map((col) => formatCellValue(row[col.key], col.format, locale))
  );
...
export function exportInventoryExcel(data: unknown[], locale: string = "es-ES"): void {
  exportToExcelPro(data as ExcelRow[], "inventario", undefined, undefined, locale);
}

export function exportProductsExcel(data: unknown[], locale: string = "es-ES"): void {
  exportToExcelPro(data as ExcelRow[], "productos", undefined, undefined, locale);
}

export function exportSalesExcel(data: unknown[], locale: string = "es-ES"): void {
  exportToExcelPro(data as ExcelRow[], "ventas", undefined, undefined, locale);
}