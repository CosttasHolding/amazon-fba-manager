...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = supabase.from("fba_shipments").select("*, purchase_orders(po_number)", { count: "exact" }).eq("org_id", orgId).order("created_at", { ascending: false }).range(from, to);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = fbaShipmentSchema.safeParse(body);
