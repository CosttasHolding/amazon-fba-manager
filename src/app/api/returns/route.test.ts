...
function queryChain(value: unknown) {
  ...
  for (const method of ["select", "eq", "maybeSingle", "insert", "single"]) {
...
describe("POST /api/returns", () => {
  ...
  it("rejects a product reference outside the active organization", async () => {
    ...
    client.from.mockReturnValueOnce(membershipQuery).mockReturnValueOnce(productQuery);

    const response = await POST(new NextRequest("http://localhost/api/returns", {
      method: "POST",
  ...
  it("rejects viewers before inserting a return", async () => {
    const membershipQuery = queryChain({ data: { role: "viewer" }, error: null });
    const client = await mocks.createClient();
