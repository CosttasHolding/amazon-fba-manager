    ...
    <div className="space-y-6 animate-fade-up">
      ...
      >
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm text-white/50 hover:text-white/70 hover:bg-white/10 transition-colors"
        >
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        ...
        <button
          type="submit"
          className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-white/40 hover:text-white/70 hover:bg-white/10 transition-colors"
        >
      ...
      {loading && (
        <div className="flex items-center justify-center py-16">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-sm text-white/40">Cargando inventario...</span>
          </div>
        </div>
      )}
      ...
      {!loading && inventory.length === 0 && (
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-12 text-center">
          <Package className="h-12 w-12 text-white/15 mx-auto mb-4" />
          <h3 className="text-lg font-semibold text-white/70 mb-1">No hay datos de inventario</h3>
          <p className="text-sm text-white/35">Agrega productos para ver su stock aqui</p>
        </div>
      )}
                ...
                {inventory.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className={tableRowClass}>
                      <td className={`${tableCellClass} font-mono text-xs text-white/40`}>
                        {p.sku}
                      </td>
                      ...
                      <td className={`${tableCellClass} text-center font-bold text-white/90 tabular-nums`}>
                        {total}
                      </td>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <p className="font-medium text-sm text-white/80">{p.name}</p>
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center">
                    <div>
                      <p className="text-[10px] text-white/30">Disponible</p>
                      <p className="font-bold text-sm text-white/70 tabular-nums">{p.stock_available || 0}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/30">Transito</p>
                      <p className="font-bold text-sm text-white/70 tabular-nums">{p.stock_inbound || 0}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/30">Warehouse</p>
                      <p className="font-bold text-sm text-white/70 tabular-nums">{p.stock_warehouse || 0}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/30">Total</p>
                      <p className="font-bold text-sm text-cyan-400 tabular-nums">{total}</p>
                    </div>
                  </div>
                </div>
              );
