...
export default function EditMemberPage({ params }: { params: { id: string } }) {
  ...
  const onSubmit = async (data: MemberFormData) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : t("members.error_update", locale));
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="max-w-2xl">
        <form onSubmit={handleSubmit(onSubmit)} className="bg-card border border-border rounded-xl p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <Select defaultValue={member.status} onValueChange={(v) => setValue("status", v as MemberFormData["status"])}>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              ...
              {saving ? t("members.saving", locale) : t("members.save_changes", locale)}
            </button>
            <button
              type="button"
              onClick={() => router.back()}
