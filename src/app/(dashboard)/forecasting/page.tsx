...
export default function ForecastingPage() {
  ...
  useEffect(() => {
    fetch("/api/forecasting")
      ...
      .catch(() => {
        toast.error(t("forecasting.error_load", locale));
        setLoading(false);
      });
  }, [locale]);
  ...
  const paginatedForecasts = useMemo(() => {
    ...
    return forecasts.slice(start, start + ITEMS_PER_PAGE);
  }, [forecasts, currentPage, ITEMS_PER_PAGE]);
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      ...
      <DataTableWrapper title={t("forecasting.title", locale)} icon={TrendingUp}>
        ...
        {forecasts.length > ITEMS_PER_PAGE && (
          <div className="p-4 border-t border-border">
            <PaginationControl
              currentPage={currentPage}
              totalPages={Math.ceil(forecasts.length / ITEMS_PER_PAGE)}
