...
export default function ProductDetailPage() {
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("products.error_load_product", locale));
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("products.error_delete", locale));
      toast.success(t("products.delete_success", locale));
      router.push("/products");
    } catch {
      toast.error(t("products.error_delete_detail", locale));
    } finally {
      setDeleting(false);
    }
  };
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-muted-foreground">{t("products.not_found", locale)}</p>
        <Button variant="outline" onClick={() => router.push("/products")}>
          {t("products.back_to_products", locale)}
        </Button>
      </div>
    );
  }
  ...
  const subtitleText = "ASIN: " + (product.asin || "N/A") + " — " + t("common.sku", locale) + ": " + (product.sku || "N/A") + (product.category ? " — " + product.category : "");
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <PageHeader
        badge={t("badge.product", locale)}
        title={product.name || t("products.unnamed", locale)}
        subtitle={subtitleText}
        breadcrumbs={[
          { label: t("nav.products", locale), href: "/products" },
          { label: product.name || t("badge.product", locale) },
        ]}
      >
            ...
            <AlertDialogContent className="bg-popover border-border">
              <AlertDialogHeader>
                <AlertDialogTitle className="text-foreground">
                  {t("products.confirm_delete_title", locale)}
                </AlertDialogTitle>
                <AlertDialogDescription className="text-muted-foreground">
                  {t("products.confirm_delete_desc", locale).replace("{name}", product.name)}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          ...
          trendValue={
            roiValue >= 20
              ? t("dashboard.trend_excellent", locale)
              : roiValue >= 0
                ? t("products.roi_acceptable", locale)
                : t("common.negative", locale)
          }
          animationDelay={150}
        />
        <KpiCard
          label={t("products.margin", locale)}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-muted-foreground">
              {t("products.profit_per_unit", locale)}
            </span>
          </div>
          <span
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-border bg-card p-6">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">
              {t("products.cost_breakdown", locale)}
            </h3>
          </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-6">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">
              {t("products.inventory_section", locale)}
            </h3>
          </div>
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">{t("products.current_stock", locale)}</span>
              <span
              ...
              >
                {product.current_stock || 0} {t("products.units", locale)}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">{t("products.min_stock", locale)}</span>
              <span className="text-sm font-medium text-foreground tabular-nums">
                {product.min_stock || 0} {t("products.units", locale)}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">{t("common.status", locale)}</span>
              <StatusBadge status={stockStatus} variant={stockVariant} />
            </div>
            {productWeight > 0 && (
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground flex items-center gap-1">
                  <Weight className="h-3 w-3" /> {t("products.weight", locale)}
                </span>
            ...
            {product.dimensions && (
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground flex items-center gap-1">
                  <Ruler className="h-3 w-3" /> {t("products.dimensions", locale)}
                </span>
                <span className="text-sm font-medium text-foreground">
                  {product.dimensions}
                </span>
      ...
      <DataTableWrapper
        title={t("products.linked_suppliers", locale)}
        actions={
          <Link
            href="/suppliers"
          ...
          >
            {t("products.view_suppliers", locale)}
        ...
        {suppliers.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-muted-foreground mb-1">
              {t("products.no_linked_suppliers", locale)}
            </p>
            <p className="text-xs text-muted-foreground/70">
              {t("products.link_suppliers_hint", locale)}
            </p>
          </div>
        ) : (
          <div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    <th scope="col" className={tableHeaderClass}>{t("products.supplier", locale)}</th>
                    <th scope="col" className={tableHeaderClass}>{t("common.country", locale)}</th>
                    <th scope="col" className={tableHeaderClass}>{t("filter.rating", locale)}</th>
                    <th scope="col" className={tableHeaderClass + " text-end"}>{t("products.unit_cost_short", locale)}</th>
                    <th scope="col" className={tableHeaderClass + " text-end"}>{t("suppliers.moq_short", locale)}</th>
                    <th scope="col" className={tableHeaderClass + " text-end"}>{t("filter.lead_time", locale)}</th>
                    <th scope="col" className={tableHeaderClass + " text-center"}>{t("products.primary", locale)}</th>
                  </tr>
                </thead>
                <tbody>
                  {suppliers.map((ps) => (
                    <tr key={ps.id} className={tableRowClass}>
                      ...
                      <td className={tableCellClass + " text-end text-muted-foreground tabular-nums"}>
                        {ps.lead_time_days ? `${ps.lead_time_days}${t("suppliers.lead_time_short", locale)}` : "N/A"}
                      </td>
              ...
              {suppliers.map((ps) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    ...
                    <div>
                      <span className="text-muted-foreground">{t("suppliers.moq_short", locale)}</span>
                      <p className="font-medium text-foreground">{ps.moq || "N/A"}</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">{t("filter.lead_time", locale)}</span>
                      <p className="font-medium text-foreground">
                        {ps.lead_time_days ? `${ps.lead_time_days} ${t("common.days", locale)}` : "N/A"}
                      </p>
                    </div>
                  </div>
                  {ps.suppliers.country && (
                    <p className="text-xs text-muted-foreground">
                      {t("products.country_label", locale)} {ps.suppliers.country}
                    </p>
                  )}
                </div>
              ))}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between text-xs text-muted-foreground/70 px-1">
        <span>
          {t("products.created_at", locale)} {product.created_at ? new Date(product.created_at).toLocaleDateString(locale === "en" ? "en-US" : "es-ES", { day: "2-digit", month: "short", year: "numeric" }) : "N/A"}
        </span>
        <span>
          {t("products.updated_at", locale)} {product.updated_at ? new Date(product.updated_at).toLocaleDateString(locale === "en" ? "en-US" : "es-ES", { day: "2-digit", month: "short", year: "numeric" }) : "N/A"}
        </span>
      </div>

      <CommentsSection entity="product" entityId={String(params.id)} />
    </div>
  );
}