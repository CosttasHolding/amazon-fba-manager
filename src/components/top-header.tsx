...
export function TopHeader({ userEmail, userName }: TopHeaderProps) {
  ...
  return (
    <header role="banner" aria-label={t("accessibility.main_header", locale)} className="hidden lg:flex sticky top-0 z-30 items-center justify-between h-14 px-8 bg-background/80 backdrop-blur-xl border-b border-border/50">
      {/* Global Search */}
      <GlobalSearch />
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="flex items-center gap-2.5 ps-3 min-h-[44px] border-s border-border/50 hover:opacity-80 transition-opacity"
          >
          ...
          {showUserMenu && (
            <div
              ...
              className="absolute end-0 top-full mt-2 w-56 rounded-xl border border-border bg-card shadow-xl shadow-black/20 py-1.5 z-50 animate-in fade-in-0 zoom-in-95 duration-150"
            >
              ...
              <div className="border-t border-border pt-1" role="group">
                <button
                  role="menuitem"
                  onClick={logout}
                  disabled={loggingOut}
