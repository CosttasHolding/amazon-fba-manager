...
function CustomTooltip({ active, payload, locale }: { active?: boolean; payload?: { payload: Record<string, unknown> }[]; locale: Locale }) {
    ...
    <div className="rounded-xl border border-border bg-card p-3 shadow-lg max-w-[220px]">
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between gap-3 text-sm">
          <span className="text-muted-foreground">{t("charts.profit_label", locale)}</span>
          <span
        ...
        <div className="flex items-center justify-between gap-3 text-sm">
          <span className="text-muted-foreground">{t("common.roi", locale)}</span>
          <span
            className={`font-display font-semibold ${
              d.roi >= 20
                ? "text-emerald-500"
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex items-center justify-center h-[300px] text-sm text-muted-foreground">
        {t("charts.profit_no_data", locale)}
      </div>
    );
  }
    ...
    <div className="w-full h-[300px]">
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          />
          <Tooltip content={<CustomTooltip locale={locale} />} cursor={{ fill: "hsl(var(--muted))", opacity: 0.3 }} />
