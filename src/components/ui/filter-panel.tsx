  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {activeCount > 0 && (
          <span className="ms-1 px-1.5 py-0.5 rounded-full bg-primary/20 text-[10px] font-bold text-primary">
            {activeCount}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute start-0 sm:start-auto sm:end-0 top-full mt-2 z-50 w-[calc(100vw-2rem)] sm:w-[460px] rounded-2xl border border-border bg-card shadow-2xl shadow-black/20 p-4 sm:p-5 space-y-4 animate-in fade-in-0 zoom-in-95 duration-150">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-all"
                aria-label={t("accessibility.close_filters", locale)}
              >
                <X className="h-4 w-4" />
              </button>
            </div>
          ...
          {sortOptions && onSortChange && (
            <div>
              <label className="text-xs text-muted-foreground mb-1.5 block">{t("filter.sort_by", locale)}</label>
          ...
          {chips.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-border">
              <span className="text-xs text-muted-foreground/70">{t("filter.active_label", locale)}</span>
              {chips.map((chip) => (
                <span
                  key={chip.key}
                ...
                >
                  ...
                  <button
                    ...
                    className="min-w-[44px] min-h-[44px] flex items-center justify-center hover:text-foreground transition-colors"
                    aria-label={t("accessibility.remove_filter", locale).replace("{label}", chip.label)}
                  >
                    <X className="h-3 w-3" />
                  </button>
                </span>
