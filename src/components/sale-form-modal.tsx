...
export function SaleFormModal({ open, onOpenChange, onSuccess }: SaleFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedProduct) {
      toast.error(t("sales.error_select_product", locale));
      return;
    }
    if (!unitsSold || parseInt(unitsSold) <= 0) {
      toast.error(t("sales.error_units_positive", locale));
      return;
    }
    if (!revenue || parseFloat(revenue) <= 0) {
      toast.error(t("sales.error_revenue_positive", locale));
      return;
    }
    ...
    try {
      ...
      toast.success(t("sales.registered", locale));
      resetForm();
      onOpenChange(false);
      ...
      <DialogContent className="max-w-lg bg-card border-border">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Select value={selectedProduct} onValueChange={handleProductChange}>
              <SelectTrigger id="product" className={inputClass}>
                <SelectValue placeholder={loadingProducts ? t("common.loading", locale) : t("common.select", locale)} />
              </SelectTrigger>
              <SelectContent>
                {products.map((p) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label htmlFor="saleDate" className={labelClass}>{t("sales.sale_date_label", locale)}</Label>
              <Input id="saleDate" type="date" value={saleDate}
                onChange={(e) => setSaleDate(e.target.value)}
                className={inputClass} />
            </div>
            <div>
              <Label htmlFor="unitsSold" className={labelClass}>{t("sales.units_sold_label", locale)}</Label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label htmlFor="revenue" className={labelClass}>{t("sales.revenue_label", locale)}</Label>
            ...
            <div>
              <Label htmlFor="amazonFees" className={labelClass}>{t("sales.amazon_fees_label", locale)}</Label>
          ...
          <div>
            <Label htmlFor="orderId" className={labelClass}>{t("sales.order_id_optional", locale)}</Label>
          ...
          {selectedProduct && unitsNum > 0 && revenueNum > 0 && (
            <div className={`rounded-xl border p-4 ${estimatedProfit >= 0 ? "border-emerald-500/20 bg-emerald-500/5" : "border-red-500/20 bg-red-500/5"}`}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium text-muted-foreground uppercase">{t("sales.estimation_label", locale)}</span>
              </div>
